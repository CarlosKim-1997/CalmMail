...
Bridge IMAP은 **자체 서명 TLS 인증서**를 씁니다. Thunderbird처럼 “예외 추가”가 없는 클라이언트는 Bridge에서 **TLS 인증서 내보내기** 후 시스템/Node 신뢰 저장소에 넣어야 할 수 있습니다. ([Bridge settings — certificate export](https://proton.me/support/comprehensive-guide-to-bridge-settings))

CalmMail 쪽에서는 향후 Bridge CA 고정(pinning) 또는 인증서 경로 설정을 검토 중입니다. 오류 메시지 전체를 이슈에 남겨 주시면 재현에 도움이 됩니다.
